                ...
                <div class="panel-footer ">
                    <div class="btu-group ">
                        <button class="btn btn-info btn-sm fa-bookmark fa" id="btn-collect"><c:if
                                test="${isCollected eq true }">√</c:if></button>
                        <c:if test="${sessionScope.user.id eq topic.user.id or sessionScope.user.role eq 'admin'}">
                            <a class="btn btn-info btn-sm glyphicon glyphicon-pencil"
                               href="${x}/topics/${topic.id}/edit" target="_blank"></a>
                            <button class="btn btn-danger btn-sm btn-delete glyphicon glyphicon-trash"
                                    data-url="${x}/topics/${topic.id}/delete"></button>
                        </c:if>
                    </div>
                </div>
            </div>
            <div class="panel panel-info">
                ...
                <ul id="comments" class="list-group-panel-body list-group">
                    <c:forEach items="${comments}" var="comment">
                        <li id="floor${comment.floor}"
                            class="list-group-item <c:if test="${comment.isLike}">list-group-item-info</c:if>">
                            <div class="media">
                                <div class="media-left">
                                    ...
                                    <a class="fa fa-thumbs-o-up btn"
                                       data-url="${x}/comments/${comment.id}/like">${comment.likeCount eq 0?'':comment.likeCount}</a>
                                </div>
                                <div class="media-body">
                                    <p class="meta"><a href="#">${comment.floor}
                                        ...
                                        <c:if test="${sessionScope.user.id eq comment.user.id or sessionScope.user.role eq 'admin'}">
                                            <a class="glyphicon glyphicon-edit btn"
                                               href="${x}/comments/${comment.id}/edit" target="_blank"></a>
                                            <a class="glyphicon glyphicon-trash btn btn-delete"
                                               data-url="${x}/comments/${comment.id}/delete"></a>
                                        </c:if>
                                    </p>
                            ...
                            <form id="form-comment" action="${x}/topics/comment" method="post">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" rows="7" name="content"
                                              id="comment-content">${comment.content}</textarea>
                                </div>
